 ...
 */
export async function detectRunningBrowsers(targets: BrowserTarget[]): Promise<RunningBrowser[]> {
  let psOut = '';
  try {
    const { stdout } = await pExecFile('ps', ['-Axo', 'rss=,command='], { timeout: 8_000, maxBuffer: 32 * 1024 * 1024 });
    psOut = stdout;
  } catch { return []; }
  ...
  for (const t of targets) {
    const appPath = await appPathForBundleId(t.bundleId);
    if (!appPath) continue; // not installed → no button
    let kb = 0;
    for (const ln of lines) {
      const m = ln.match(/^\s*(\d+)\s+(.*)$/);
      if (!m) continue;
      if (m[2].includes(appPath)) kb += Number(m[1]) || 0;
    }
...
export async function restartBrowser(
  ...
  const isRunning = opts.isRunning ?? (async (id) => (await appPathForBundleId(id)) !== '' && await bundleHasProcess(id, run));
  ...
  if (!gone) {
    ...
    return { ok: false, quit: false, relaunched: false, error: '浏览器未在超时内退出（可能有未保存的弹窗），已放弃，未强杀' };
  }
...
async function bundleHasProcess(
  bundleId: string,
  run: (file: string, args: string[]) => Promise<{ stdout: string }>,
): Promise<boolean> {
  const appPath = await appPathForBundleId(bundleId);
  if (!appPath) return false;
  try {
    const { stdout } = await run('ps', ['-Axo', 'command=']);
    return stdout.split('\n').some(ln => ln.includes(appPath));
  } catch { return true; } // unknown ⇒ assume still running (safer: don't relaunch a live app)
}
